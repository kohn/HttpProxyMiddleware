...
class HttpProxyMiddleware(object):
    ...
    def inc_proxy_index(self):
        ...
        if self.len_valid_proxy() <= self.fixed_proxy: # 如果代理列表中只有fixed_proxy个有效的代理的话重置为valid
            self.reset_proxyes()
            
        if self.len_valid_proxy() < self.extend_proxy_threshold: # 代理数量不足, 抓取新的代理
    ...
    def set_proxy(self, request):
        ...
        logger.debug("%s UsingProxy: %s" % (request, proxy["proxy"]))
        if proxy["proxy"]:
    ...
    def dump_valid_proxy(self):
        ...
        with open(self.proxy_file, "w") as fd:
            for i in range(self.fixed_proxy, len(self.proxyes)):
                p = self.proxyes[i]
                if not p["valid"] and p["count"] < self.dump_count_threshold: # valid==False并且只用到没几次就invalid的代理不再保留
                    pass
                else:
                    fd.write(p["proxy"][7:]+"\n") # 只保存有效的代理
    ...
    def process_exception(self, request, exception, spider):
        ...
        处理由于使用代理导致的连接异常
        """
        logger.debug("%s exception: %s" % (request, exception))
        request_proxy_index = request.meta["proxy_index"]
